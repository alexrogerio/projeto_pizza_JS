...
pizzaJson.map((item,index)=>{
    ...
    pizzaItem.querySelector('a').addEventListener('click',(e)=>{
       ...
       // Preenchendo o modal com as informações da pizza clicada
...
cs('.pizzaInfo--size').forEach((item,indexItem)=>{
    ...
    item.addEventListener('click',()=>{
        let key = parseInt(modalKey);
        c('.pizzaInfo--size.selected').classList.remove('selected');
        item.classList.add('selected');
        c('.pizzaInfo--actualPrice').innerHTML = `R$ ${pizzaJson[key].price[indexItem].toFixed(2)}`;
...
function updateCart(){
    ...
    if(cart.length > 0){
        ...
        for(let i in cart){
            ...
            cartItem.querySelector('.cart--item-nome').innerHTML = `${pizzaItem.name} (${pizzaSizes[cart[i].size]}) - R$ ${pizzaItem.price[cart[i].size].toFixed(2)}`;
